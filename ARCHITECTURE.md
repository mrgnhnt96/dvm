...
The palette is **semantic**, not decorative: `ok` green, `warn` yellow, `FAIL`
red, section headings and result lines bold, commands the user is meant to RUN
cyan, and everything supporting them dimmed. Dimming is the load-bearing half —
what makes `dvm doctor` skimmable is pushing context BACK, not adding colours to
look at, so when in doubt dim rather than colourise.

`DvmContext.styles` is the only place this happens, and it offers a helper per
ROLE rather than per colour, for the same reason `display` is one function: with
`green(...)` in reach, `ok` ends up green in one command and bold-green in the
next, and the palette stops being changeable in one edit.

Colour is OFF unless every one of these holds, and `Styles.enabled` states the
precedence:

- `NO_COLOR` is unset or empty (https://no-color.org/),
- `TERM` is set and is not `dumb`,
- the sink is a real terminal — `DvmContext.outIsTerminal`, which `lib/dvm.dart`
  computes as `out == null && stdout.hasTerminal`, so an INJECTED sink is never
  assumed to be one and the whole test suite runs uncoloured without opting out.

`--color=auto|always|never` sits beside `-v` as a top-level flag. `always` beats
`NO_COLOR` and `TERM=dumb` both — the user asked for colour in this run, which is
what somebody piping into `less -R` needs; `never` beats everything the other way.

Two things must NEVER carry an escape sequence, and they are the same two the
path rule carves out, for the same reason:

- **`dvm which --path` and the first line of `dvm which`.** They are read with
  `$(dvm which)`. An escape sequence in there is a path that does not exist.
- **Anything dvm WRITES to a file** — `.dvmrc`, the `.dvm/dart_sdk` target, and
  above all the PATH line added to a startup file. A colour code in `~/.zshrc`
  would be a genuinely nasty bug. Styling happens where a line is PRINTED and
  nowhere near where one is written; tests pin both.

With colour off the output is byte-identical to what it was before any styling
existed. `test/commands/color_off_golden.txt` is that "before", generated by
`tool/generate_golden.dart` against the commit that predates it, and
`color_test.dart` checks both directions: colour off equals the golden, and
colour on with the escapes stripped back out equals it too.
...
**The prerelease filter is a whitelist, not a ranking.** The release scan takes the
newest entry that is NOT flagged prerelease and that carries the expected asset, and
there is no flag anywhere that widens it. `install.sh`'s `pick_tag` and the CLI's
`Updater.latestRelease` apply the same two filters — not a prerelease, and a `v<x>.<y>.<z>`
tag — so neither an install nor an update can land on unreleased code. When nothing
qualifies, both refuse and say so rather than falling back to something the user did not
ask for.

`release.yml` is `workflow_dispatch`-only: a release goes out only after a human chose to
publish it, and dispatch being its sole trigger is what enforces that. Nothing publishes
to the releases page on a push.
